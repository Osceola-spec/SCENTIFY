                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-[10px] font-mono uppercase tracking-widest text-slate-400 mb-3 font-bold">
                                    Galeri Foto Saat Ini
                                </label>
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                                    @foreach ($product->images as $img)
                                        <div class="relative group rounded-xl overflow-hidden border border-slate-200 bg-slate-50 aspect-square" id="existingImg{{ $img->id }}">
                                            <img src="{{ $img->url }}" class="w-full h-full object-cover">
                                            {{-- Badge primary --}}
                                            @if ($img->is_primary)
                                                <span class="absolute top-1.5 left-1.5 bg-amber-500 text-black text-[9px] font-bold px-2 py-0.5 rounded-full">Utama</span>
                                            @else
                                                {{-- Tombol hapus hanya untuk non-primary --}}
                                                <button type="button"
                                                        onclick="markDeleteImage({{ $img->id }}, this)"
                                                ...
                                                <input type="checkbox" name="delete_images[]" value="{{ $img->id }}"
                                                    id="deleteImg{{ $img->id }}" class="hidden">
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                                <p class="text-[10px] text-slate-400 mt-2">
                                    <i class="fas fa-info-circle mr-1"></i> Hover foto lalu klik <span class="text-rose-500 font-semibold">✕</span> untuk menandai hapus. Foto utama tidak dapat dihapus.
                                </p>
                            </div>
                            @endif
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-[10px] font-mono uppercase tracking-widest text-slate-400 mb-2 font-bold">
                                    Tambah Foto Baru <span class="text-slate-300">(Opsional, maks. 5 foto)</span>
                                </label>

                                <div id="dropZone"
                                    class="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50/30 transition-all duration-200"
                                ...
                                <p id="imageCounter" class="text-[11px] text-slate-400 mt-2 hidden">
                                    <span id="imageCountNum">0</span> foto baru dipilih
                                </p>
                            </div>
                        </div>
                    </div>
