...
<div class="p-6 max-w-2xl">
    ...
    <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST">
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                ...
                <select name="discount_type" class="w-full border px-3 py-2 rounded">
                    <option value="percent" {{ $promotion->discount_type === 'percent' ? 'selected' : '' }}>Persen (%)</option>
                    <option value="fixed" {{ $promotion->discount_type === 'fixed' ? 'selected' : '' }}>Nominal (Rp)</option>
                </select>
            </div>
            <div>
        ...
        <div class="mt-3">
            <label class="inline-flex items-center"><input type="checkbox" name="applies_to_all" {{ $promotion->applies_to_all ? 'checked' : '' }}> Berlaku untuk semua produk</label>
        </div>
        ...
        <div class="mt-4">
            <label class="inline-flex items-center"><input type="checkbox" name="is_active" {{ old('is_active', $promotion->is_active) ? 'checked' : '' }}> Aktif</label>
        </div>

        <div class="mt-4">
            <button class="px-4 py-2 bg-amber-500 text-white rounded">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection
